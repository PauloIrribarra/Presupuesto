...
function App() {
	...
	return (
		<main className='min-h-screen overflow-x-hidden px-3 py-4 text-slate-900 sm:px-6 sm:py-6 lg:px-8'>
			<div className='mx-auto flex w-full max-w-7xl flex-col gap-5 sm:gap-6'>
				<header className='flex min-w-0 flex-col gap-4 border-b border-slate-200 pb-5 sm:pb-6 md:flex-row md:items-end md:justify-between'>
					<div className='min-w-0'>
						<p className='text-sm font-semibold uppercase tracking-wide text-emerald-700'>
							Presupuesto personal
						</p>
						<h1 className='mt-2 text-2xl font-semibold leading-tight text-slate-950 sm:text-3xl md:text-5xl'>
					...
					<div className='flex w-full min-w-0 flex-col gap-3 md:max-w-xs'>
						...
						{SHOW_RESET_BUTTON && (
							<button
								type='button'
								onClick={resetData}
								className='rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-600 transition hover:border-rose-300 hover:bg-rose-100'
							>
					...
					<div className='min-w-0 space-y-5 sm:space-y-6'>
						...
						>
							...
							<div className='mt-4 grid gap-4'>
								...
								<button className='rounded-lg bg-emerald-700 px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-800'>
									Agregar movimiento
								</button>
							</div>
						</form>
		...
		<article className='min-w-0 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-5'>
			...
			<p
				className={`mt-2 break-words font-semibold leading-tight ${
					strong
						? 'text-2xl text-emerald-800 sm:text-3xl'
						: 'text-xl text-slate-950 sm:text-2xl'
				}`}
			>
				{value}
			</p>
	...
	return (
		<label className='flex flex-col gap-2 text-sm font-medium text-slate-700'>
			...
			<input
				...
				className='min-w-0 rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100'
			/>
		</label>
	);
}
							...
							<div key={category} className='rounded-lg border border-slate-100 p-3'>
								...
								>
									<div className='mb-2 grid gap-1 text-sm sm:flex sm:items-center sm:justify-between sm:gap-3'>
										...
										<span className='pl-6 text-xs font-semibold text-slate-500 sm:shrink-0 sm:pl-0 sm:text-sm'>
								...
								{isOpen && (
									<div className='mt-3 divide-y divide-slate-100 rounded-lg bg-slate-50'>
										{movements.map((movement) => (
											...
											>
												...
												<span className='font-semibold text-slate-950 sm:text-right'>
													{CLP.format(movement.amount)}
												</span>
											</div>
										))}
	...
	return (
		<div className='relative flex flex-col gap-2 text-sm font-medium text-slate-700'>
			...
			<button
				type='button'
				onClick={onToggle}
				className='flex min-w-0 items-center justify-between rounded-lg border border-slate-300 bg-white px-3 py-2 text-left font-normal text-slate-900 outline-none transition hover:border-slate-400 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100'
				aria-expanded={isOpen}
			>
			...
			{isOpen && (
				<div className='absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden rounded-lg border border-slate-200 bg-white p-1 shadow-lg'>
	...
	return (
		<article className='grid gap-3 p-4'>
			<div className='flex min-w-0 items-start justify-between gap-3'>
				<div className='min-w-0'>
					<p className='text-xs font-medium text-slate-500'>{movement.date}</p>
			...
			<div className='grid gap-2'>
				...
				<div className='flex items-center justify-between gap-3'>
					<p className='break-words text-lg font-semibold leading-tight text-slate-950'>
						{isExpense ? '-' : '+'}
						{CLP.format(movement.amount)}
					</p>
					<button
						...
						className='shrink-0 rounded-md px-2 py-1 text-xs font-semibold text-slate-500 hover:bg-slate-100 hover:text-slate-900'
					>
						Eliminar
					</button>
				</div>
